...
func (s *TestSuite) TestProcessDeletion(c *C) {
	...
	for i := 0; i < count; i++ {
		...
		go func(i int) {
			...
			assertProcessCreation(c, s.pm, name, binary)
...
func (s *TestSuite) TestProcessReplace(c *C) {
	...
	for i := 0; i < count; i++ {
		...
		go func(i int) {
			...
			binary := TestBinary
			assertProcessCreation(c, s.pm, name, binary)
			assertProcessReplace(c, s.pm, name, binary+"-new")
...
func (s *TestSuite) TestProcessReplaceMissingBinary(c *C) {
	...
	for i := 0; i < count; i++ {
		...
		go func(i int) {
			...
			// this will deadlock in the current implementation
			assertProcessReplace(c, s.pm, name, "fail-missing-binary")
		}(i)
	}
	wg.Wait()
}
...
func (s *TestSuite) TestProcessReplaceDuringDeletion(c *C) {
	...
	for i := 0; i < count; i++ {
		...
		go func(i int) {
			...
			// this will throw a nil pointer due to the current implementation
			assertProcessReplace(c, s.pm, name, binary+"-new")
		}(i)
	}
	wg.Wait()
